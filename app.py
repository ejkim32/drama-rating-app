...
from preprocess_v2 import (
    load_data, build_training_objects, align_user_to_training, clean_cell_colab
)
...
with tabs[6]:
    ...
    if st.button("GridSearch 실행"):
        # ✅ 재현성: shuffle 여부를 옵션으로
        cv_obj = KFold(n_splits=int(cv), shuffle=cv_shuffle, random_state=(SEED if cv_shuffle else None))
...
with tabs[8]:
    ...
    if predict_btn:
        ...
        # 🔹 학습 스키마에 맞춰 정렬/채움
        user_base = align_user_to_training(
            user_raw, X_colab_base, drop_cols, state=st.session_state, ref_df_mlb=df_mlb
        )
